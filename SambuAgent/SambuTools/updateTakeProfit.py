...
async def updateTakeProfit(
    private_key: str, market_id: int, trade_side: bool, new_take_profit_price: int
) -> dict:
    """
    Update the take profit price for a specific market.
    ...
    Args:
        ...
        new_take_profit_price (int): The new take profit price.
    ...
    private_key_hex = private_key  # privateKey
    if private_key_hex.startswith("0x"):
        private_key_hex = private_key_hex[2:]
    private_key_bytes = bytes.fromhex(private_key_hex)
    account = Account.load_key(private_key_bytes)
    API_URL = f"{os.environ.get('KANA_BASE_URL')}/updateTakeProfit"
    PARAMS = {
        "marketId": market_id,
